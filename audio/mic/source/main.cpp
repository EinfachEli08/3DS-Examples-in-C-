...
int main()
{
    ...
    u32 micbuf_pos = 0;
    u8* micbuf = (u8*)memalign(0x1000, micbuf_size);
    ...
    if (initialized)
        printf("Hold A to record, release to play.\n");
    ...
    while (aptMainLoop())
    {
        ...
        if (initialized)
        {
            ...
            if (hidKeysUp() & KEY_A)
            {
                ...
                if (R_SUCCEEDED(GSPGPU_FlushDataCache(audiobuf, audiobuf_pos)) && R_SUCCEEDED(csndPlaySound(0x8, SOUND_ONE_SHOT | SOUND_FORMAT_16BIT, 16360, 1.0, 0.0, (u32*)audiobuf, NULL, audiobuf_pos)))
                    printf("Now playing.\n");
                else
